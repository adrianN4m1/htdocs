...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt->execute() === TRUE) {
        ...
        $stmt_branch->bind_param("ii", $idbranch, $user_id);
        if ($stmt_branch->execute() === TRUE) {
            echo '<script>alert("User registered in ' . $idbranch . '!"); window.location.href = "' . $_SERVER['HTTP_REFERER'] . '";</script>';
